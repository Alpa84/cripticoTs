...
function TourWrapper({children, functions, general, tutName, componentClass}: Props) {
  ...
  const [doFlash, setFlash] =  useState(false)
  ...
  return (
    <div
      ...
      { show && (
        <>
          <div className={doFlash ? 'tourExplanation tourFlash' : 'tourExplanation'}>
            ...
            <div className='tourArrows'>
              {general.mobileStep > 0 && (
                <svg
                  className='tourArrow'
                  onClick={() => functions.setStepFromTour(general.mobileStep - 1)}
