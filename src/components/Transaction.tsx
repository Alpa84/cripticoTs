...
function Transaction({
  ...
  let block = general.editableChain
    ? general.editableChain[blockIndex]
    : general.chain[blockIndex]
  let transaction = block.transactions[transactionIndex]
  let hint = general.wallets[transaction.gives]
    ? `(${general.wallets[transaction.gives].alias})`
    : `(no wallet with that address)`
  return (
    <div>
      <div>
        {transaction.gives !== Mined && <span className="hint">{hint}</span>}
        <Input
          text="giver's address"
          value={transaction.gives}
          onChange={(event) =>
            functions.dispatch({
        ...
        {general.wallets[transaction.receives] ? (
          <span className="hint">{`(${
            general.wallets[transaction.receives].alias
          })`}</span>
        ) : (
        ...
        <Input
          text="signature"
          value={transaction.signature ? transaction.signature : ''}
          onChange={(event) =>
            functions.dispatch({
